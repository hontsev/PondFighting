 ...
 */
Pond.Battle.TIME_LIMIT = 30 * 1000;
...
Pond.Battle.update = function() {
  ...
  setTime((Pond.Battle.endTime_-Date.now())/1000);
    ...
    if (missile.range - missile.progress < Pond.Battle.MISSILE_SPEED / 2) {
      ...
      for (var j = 0, avatar; avatar = Pond.Battle.AVATARS[j]; j++) {
        ...
        if (damage > 0) {
		  ...
		  print((avatar.visualizationIndex+1)+"号选手："+avatar.name+" 被击中了，损失了"+Math.round(damage*100)/100+"点生命。");
  ...
  for (var i = 0, avatar; avatar = Pond.Battle.AVATARS[i]; i++) {
    ...
    if (avatar.speed > 0) {
      ...
      if (avatar.loc.x < 0 || avatar.loc.x > 100 ||
		...
		print((avatar.visualizationIndex+1)+"号选手："+avatar.name+" 撞了墙，损失了"+Math.round(damage*100)/100+"点生命。");
        ...
        if (closestAfter < Pond.Battle.COLLISION_RADIUS &&
		  ...
		  print((avatar.visualizationIndex+1)+"号选手："+avatar.name+" 撞了 "+(neighbour.visualizationIndex+1)+"号选手："+neighbour.name+"，双方都损失了"+Math.round(damage*100)/100+"点生命。");
  ...
  for (var j = 0; j < Pond.Battle.STATEMENTS_PER_FRAME; j++) {
    ...
    for (var i = 0, avatar; avatar = Pond.Battle.AVATARS[i]; i++) {
      ...
      try {
		...
		print((avatar.visualizationIndex+1)+"号选手："+avatar.name+" 的脚本在运行时出了问题！只好提前中止这场比赛");
        avatar.die();
      }
      Pond.Battle.currentAvatar = null;
    }
